...
Now we want to transfer the loaded text source into a corpus object of the `quanteda`-package. First we load the package.
...
as.character(texts(corpus)[1])
```
```{r eval=T, echo=F}
paste0(substring(as.character(texts(corpus)[1]), 0, 120), "...")
```
