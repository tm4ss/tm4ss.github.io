...
annotated_corpus <- lapply(texts(corpus)[1:10], annotateDocuments)
...
sample_corpus <- sapply(texts(corpus)[1:1000], annotateDocuments, pos_filter = c("NNP", "NNPS"))
...
binDTM <- filtered_corpus %>% tokens(what = "fasterword") %>% tokens_tolower() %>%
  tokens_ngrams(1) %>% dfm() %>% dfm_weight(scheme = "boolean")
