...
corpus <- corpus(textdata$text, docnames = textdata$doc_id, docvars = data.frame(year = substr(textdata$date, 0, 4)))
```
...
corpus_edit <- corpus_sentences %>% tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% tokens_tolower() %>% 
...
colnames(binDTM) <- colnames(binDTM) %>% stringi::stri_replace_all_regex("[^_a-z]", "")

binDTM <- dfm_compress(binDTM, "features")
```
