...
corpus_edit <- corpus %>% tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% tokens_tolower() %>%tokens_ngrams(1)
```
...
corpus_edit_sw <- corpus %>% tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% tokens_tolower() %>% 
  tokens_ngrams(1) %>% tokens_remove(pattern = stopwords())


print(paste0("1: ", substr(paste(corpus_edit_sw[4963],collapse = " "), 0, 400), '...'))
...
corpus_edit_bigrams <- corpus %>% tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% tokens_tolower() %>% 
 tokens_ngrams(1:2) %>% tokens_remove(pattern = stopwords())


print(paste0("1: ", substr(paste(corpus_edit_bigrams[4963],collapse = " "), 0, 400), '...'))

minimumFrequency <- 5

DTM <- corpus_edit_bigrams  %>% dfm() %>% 
  dfm_trim(min_docfreq = minimumFrequency,max_docfreq = Inf)
...
# Stemming
corpus_edit_bigrams_lemma <- corpus %>% tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% tokens_tolower() %>% 
  tokens_replace(., lemmaData$V1, lemmaData$V2) %>% tokens_tolower() %>%
  tokens_ngrams(1:2) %>% tokens_remove(pattern = stopwords())


print(paste0("1: ", substr(paste(corpus_edit_bigrams_lemma[4963],collapse = " "), 0, 400), '...'))

minimumFrequency <- 2

DTM <- corpus_edit_bigrams_lemma  %>% dfm() %>% 
  dfm_trim(min_docfreq = minimumFrequency,max_docfreq = Inf)
